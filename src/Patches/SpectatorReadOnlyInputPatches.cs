...
internal static class SpectatorEndTurnPatch
{
    private static bool Prefix()
    {
        return SpectatorActionRequestPatch.RejectLocalAction("end turn");
    }
}

[HarmonyPatch(typeof(MapSelectionSynchronizer), nameof(MapSelectionSynchronizer.PlayerVotedForMapCoord))]
internal static class SpectatorMapVotePatch
{
    private static bool Prefix()
    {
        return SpectatorActionRequestPatch.RejectLocalAction("map selection");
    }
}
