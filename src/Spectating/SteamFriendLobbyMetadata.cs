...
internal static class SteamFriendLobbyMetadata
{
    ...
    internal static async Task<bool> IsRunningCompatibleSession(ulong friendSteamId)
    {
        ...
        var isCompatible = enabled == "1" && phase == "running" && protocol == ModInfo.ProtocolVersion.ToString();
        Log.Info($"[{ModInfo.Id}] Friend {friendSteamId} lobby {lobbyId.m_SteamID}: spirewatch='{enabled}', phase='{phase}', protocol='{protocol}', spectator={isCompatible}.");
        return isCompatible;
    }
